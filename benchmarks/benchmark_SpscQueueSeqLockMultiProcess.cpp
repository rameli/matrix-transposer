...
static void DoSetup(const benchmark::State& state)
{
    ...
    else if (pid == 0)
    {
        ...
        // pQueue = std::make_unique<SpscQueueSeqLock>(gProducerPid, SpscQueueSeqLock::Role::Consumer, CAPACITY, "");
    ...
    else
    {
        ...
        gIsProducer = true;
        pQueue = std::make_unique<SpscQueueSeqLock>(gProducerPid, SpscQueueSeqLock::Role::Producer, CAPACITY, "");
...
BENCHMARK(BM_SpscQueueSeqLockEnqueueDeque)
    ->Setup(DoSetup)
    ->Teardown(DoTeardown)
    ->ArgName("Item Count")
